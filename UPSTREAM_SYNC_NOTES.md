...
- **3 ⏳ TODO** (not yet attempted, lower-risk than the deferred set)
...
| B. Weenie / Spell / Monster editor | ~7 commits, new editor surfaces | Med-High | `3457ea7` SpellEditor copy ✅ ported 2026-04-29. `4dc3983` ✅ ported 2026-04-29 in full (Weenie property enums + DID/int pickers + Weenie editor Add/Remove/Create-New + dungeon/landscape browser wire-up; landscape favorites/paging hunks deferred — depend on a paging refactor that isn't in this fork). `d780244` 🟡 spell-editor UX slice ✅ ported 2026-04-29 (presets/undo/validation/toast/paging); landscape+dungeon parts and the `InstancePlacementsPanelView` styling tweak deferred behind `d512ef2`. `55604d2` ✅ ported 2026-04-29 as the d780244 merge resolution (the original "likely blocked on InstancePlacementsPanelView" prediction was wrong: only 1 of d780244's 22 files actually touches InstancePlacementsPanelView, and it's purely a selected-state styling change — the spell-editor portion is fully independent). Remaining: `239c0c1`/`92fafff`/`d512ef2` (Monster editor + ACE DB instance placements — new editor surfaces). `d512ef2` was attempted on a side branch 2026-04-29 and aborted: 8 conflicts including 6 regions in `AceDbConnector.cs` alone — not safe to land without in-game verification, and the doc's own guidance says structural commits should be re-implemented from intent, not cherry-picked. |
