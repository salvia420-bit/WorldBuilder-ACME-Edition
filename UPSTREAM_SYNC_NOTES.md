...
Last full audit: **2026-04-26** (incremental: 2026-04-30 — third pass: Wave F (Layout editor overhaul) ✅ landed — `LayoutEditorViewModel` gains `LayoutDatDocument` + `PortalDatDocument` plumbing through `Init(project)`, `SaveLayoutToProject` [RelayCommand] + `LayoutListItem.HasProjectOverride` flag (`*` suffix in the list); `LayoutDetailViewModel` now holds a mutable `WorkingLayout` (cloned via `LayoutDescBinary.Clone`), all-text geometry edit fields (X/Y/W/H/Z/L/T/R/B + ReadOrder + LayoutWidth/Height + PrimarySurfaceHex + NewTemplateImageHex), `ApplyLayoutSize`/`ApplyElementGeometry`/`ApplyPrimarySurface`/`AddTemplateImage`/`ReplaceUiRenderSurfaceAsync` commands, `LoadPreviewTexturesAsync` background decode via `SurfaceBrowserViewModel.DecodeSurfaceToBitmap`; `ElementTreeNode` is now an `ObservableObject` with `Source`-backed `=>` properties + `SetPosition`/`SetBounds`/`NotifySourceMutated`/`RefreshStateRows` + `DefaultStateHex` + `StateRows` collection; `LayoutPreviewCanvas` gains pointer pressed/moved/released, hit-testing for body + 4 corners + 4 edges with cursor swaps, drag-resize math (clamped to canvas with `ClampToLayout`), 8 visible resize handles, image draws via `ElementTextures` lookup; `LayoutEditorView.axaml` adds Save-to-project status-bar button + Layout-canvas size box + element-geometry edit grid + Edges grid + Textures/media section (Apply primary, Add template, Replace in-place) + UI-states-and-media `ItemsControl` over `StateRows`; `LayoutEditorView.axaml.cs` rewires subscribe/detach into `AttachDetail`/`AttachElement` + `OnSelectedElementPropertyChanged` → `SyncGeomTextsFromElement`. The only local divergence: `TextureImportService.TryOverwriteUiRenderSurface(path, id, portalDoc)` keeps its 3-arg `34c612b` signature; threaded `_portalDoc` through `LayoutDetailViewModel` constructor + `CanReplaceUiRenderSurface` gate. `LayoutEditorViewModel` ctor gains optional `TextureImportService? textureImport` (DI singleton injected). Builds clean across Shared + WorldBuilder + Terminal + Tests; 92/93 tests pass (same pre-existing Linux path-separator test). In-game verification of the new geometry/texture edits + drag-resize handles + Save-to-project flow still pending. Cluster B + Slice 4 now both fully landed except for the deliberately-skipped d512ef2 ObjectBrowser-side weenie UX shortcut (intentional). Earlier 2026-04-30 — second pass: big GenerateWorld upgrade ✅ landed (`LandblockDocument.ClearAllStatics`, real `SkipDatStatics` early-return in `LandblockDocument.InitInternal`, `DocumentManager.ResetWorldDocumentsAsync`, FreshStart [RelayCommand] + confirmation dialog, full GenerateWorld with progress dialog + minimap pre-sample + Task.Run + warmup queue + ShowWorldGenSummary + Export Towns CSV + GetTownTelelocAnchor + BuildAceTeleLoc); `af985b2` `.github/release.yml` ✅ ported (the workflow-yml halves still 🔧 PERMISSIONS-blocked). Earlier 2026-04-30 — `d780244` dungeon + landscape selection slice ported now that `d512ef2` is in: `DungeonObjectRaycast` second pass + `DungeonSelectionManager.SelectInstancePlacement` + `Dungeon/Tools/SelectTool` Delete/Escape paths + `InstancePlacementsPanelView.axaml` selected-state styling on the dungeon side; `ObjectSelectionState.HasSelectedOutdoorInstance` + `TerrainEditingContext.OutdoorInstancesChanged` event + `GameScene._outdoorInstanceRenderObjects` + `ObjectRaycast` third pass + `LandscapeEditorViewModel.RefreshOutdoorInstanceRenderObjects` + `SelectSubToolViewModel` outdoor-instance click + delete on the landscape side. ObjectBrowser-side `PlaceWeenieAsInstance` toggle deliberately skipped — local lacks the d512ef2 `ShowWeenies` shortcut, so the gate is unreachable; the AceInstancesPanel + WeeniePickerViewModel already drive the same write through OutdoorInstancePlacements. Builds clean, 92/93 tests pass. Earlier 2026-04-29 — `44f47f8` gizmo improvement and `fa7c58c` surface-browser paging + terrain conformance ported; later same day `b9ffe3e` dungeon view fill ported, `38b22c2` texture fixes / docs ported partially, and `15cb9ed` logging + dat-export fix ported in full — closed the audit window. Also same day `3457ea7` SpellEditor copy spell ported in full, `39d68d7` WorldGen tweaks ported partially, and `4dc3983` ported in full across two passes — first the dungeon object-browser slice, then the property-pickers + entity-enums + Weenie-editor + remaining dungeon/landscape wire-up. Final 04-29 pass: `67912c1` SQL-export reposition reclassified ✅ PORTED — verification showed the SQL connector + `InstanceRepositionService` + `RepositionContext` + `LandblockInstanceRecord` + DB-export wiring were all already in place from prior slice work and locally extended with indoor-building delta support; doc had it as TODO. Also ported `d780244` spell-editor UX improvements (presets, undo, validation, toast, paging, sort, change tracking) — spell-only slice; the landscape/dungeon/InstancePlacementsPanelView pieces depend on `d512ef2` and stay deferred. Auto-merge produced two `CopySpell` methods (upstream's new one + our 3457ea7 one); resolved with `55604d2`'s pattern (delete the duplicate, hoist DB-clone block into `CloneDbSpellRecord` helper) — so `55604d2` lands ✅ PORTED in the same pass. **2026-04-29 final**: `239c0c1` (new monster builder) + `92fafff` (monster editor improvements) ported — Cluster B almost closed, only `d512ef2` (ACE DB instance placements) remains TODO. The `239c0c1` cherry-pick auto-merged most of the file but conflicted on `LandscapeEditorViewModel.GenerateWorld` (where upstream's pre-commit base differs from ours: upstream had FreshStart + the big GenerateWorld upgrade; we have the simpler version). Resolution: dropped the entire upstream GenerateWorld block and kept ours — Monster editor doesn't depend on it. `GameScene` conflict bringing in duplicate Wave-G content was resolved by keeping only the two genuinely-new methods (`InvalidateEnvCellsForLandblock`, `PreviewBuildingInterior`); `QueueModelWarmup` + `_pendingModelWarmup` were skipped (consistent with the deliberate Wave G skips). Added a stub `DocumentManager.SkipDatStatics { get; set; }` property so the cherry-picked GameScene retry-loop guard compiles. Also backfilled `38b22c2`'s previously-skipped Monster pieces: `ReplaceRenderSurfaceAsync` [RelayCommand] + the GUI button in `MonsterEditorView.axaml` + the `MonsterPreviewView.axaml` inner-Panel `#0d0a14` background.
...
**2026-04-30 follow-up — big GenerateWorld upgrade unblocked.** With `Scene.QueueModelWarmup` already landed (Wave G backfill on 2026-04-29) and `TerrainDocument.ApplyBulkImport` long since in place, the only remaining prereqs were tiny: `LandblockDocument.ClearAllStatics` (5 lines: clear list + flip `_loadedFromProjection = true` + MarkDirty so the empty state survives a reload) and the real `SkipDatStatics` early-return guard at the top of `LandblockDocument.InitInternal` (the property existed but the LandblockDocument consumer was missing — only `GameScene` was reading it). Both landed. Then ported: `DocumentManager.ResetWorldDocumentsAsync` (active landblock docs cleared via `ClearAllStatics`, inactive landblock + all dungeon docs deleted from storage; uses local's `ListDocumentIdsAsync(prefix)` instead of upstream's `GetDocumentIdsByPrefixAsync`); the FreshStart `[RelayCommand]` + `ShowFreshStartConfirmation` dialog (resets all terrain to WaterDeepSea + clears statics + deletes dungeons); the big `GenerateWorld` upgrade (per-LB minimap pre-sample → progress dialog with live status text → `Task.Run`-backed Generate → `SkipDatStatics=true` + ResetWorldDocuments → terrain bulk-import → buildings + decorations into LandblockDocuments → `SkipDatStatics=false` re-enable → warmup queue for every unique building/decoration model id → `ShowWorldGenSummary`); `ShowWorldGenSummary` dialog with Settlements list + Export Towns CSV button; `ExportTownsCsv` (writes `Name,Size,Buildings,LandblockHex,OutdoorCellHex,TeleLoc` rows using `Avalonia.Platform.Storage.SaveFilePickerAsync`); `GetTownTelelocAnchor` helper (centroid of buildings tagged with the town name, falling back to `WorldCenter`) and `BuildAceTeleLoc` (`@teleloc`-format outdoor coords). The previously-stub `DocumentManager.SkipDatStatics` now has real behaviour. `Avalonia.Layout.Orientation` was qualified at the StackPanel call sites to disambiguate from `WorldBuilder.Lib.Docking.Orientation`. Builds clean across Shared + WorldBuilder + Terminal + Tests; 92/93 tests pass (same pre-existing Linux path-separator test). FreshStart deliberately wired only as a [RelayCommand] (not a toolbar button — upstream also leaves it command-only). In-game verification of the FreshStart confirmation flow + WorldGen progress dialog + Export Towns CSV still pending.

Same pass: ported `af985b2`'s `.github/release.yml` (the GitHub native "Generate release notes" categories config — features / bug fixes / docs / maintenance / catch-all). The two `.github/workflows/*.yml` modifications from the same commit remain blocked on the OAuth token's missing `workflow` scope; reclassified `af985b2` as 🟡 PARTIAL because the release-config piece is no longer pending.
...
| `af985b2` | 03-10 | Changelog from commit messages; add `release.yml` | 3 | +58/-32 | 🟡 PARTIAL | `.github/release.yml` (GitHub native release-notes categories config) ✅ ported 2026-04-30 — sits next to local's existing `changelog-config.json` (different action, no conflict). The two `.github/workflows/*.yml` modifications (`BuildEdge.yml` switching from `gh api generate-notes` to `git log`, plus the matching `MakeStableReleaseDraft.yml`) remain 🔧 PERMISSIONS-blocked: OAuth token lacks `workflow` scope. Push the workflow halves from a session with `workflow` scope. |
...
| `f26345e` | 03-22 | weenie editor, layout overhaul, transform gizmo, world gen, mesh import/export, texture | 86 | +10 452/-661 | ✅ PORTED | See "f26345e split into 4 slices" below. Slices 1-3 (headless) ✅ PORTED. Slice 4 waves A-G all ✅ — Wave F (Layout editor overhaul) ✅ landed 2026-04-30 (4-file synchronized port: VM + PreviewCanvas + axaml + axaml.cs; local was byte-identical to upstream's pre-f26345e baseline so the diff applied cleanly; only divergence was threading PortalDatDocument through LayoutDetailViewModel for the local 3-arg `TryOverwriteUiRenderSurface(path, id, portalDoc)` signature from `34c612b`). |
...
| `d780244` | 04-11 | improve spell editor UX | many | medium | 🟡 PARTIAL | Cluster B. Spell-editor slice ✅ ported 2026-04-29 (`SpellEditorViewModel.cs` +505/-50, `SpellEditorView.axaml` +185/-...): presets dropdown for new spells (Blank/Basic Bolt/Basic Buff/Basic Portal), undo stack with `SpellUndoAction` discriminated kinds, validation messages with `HasBlockingIssues`/`CanSave` gating, toast notifications with optional action, paging (`SpellPageSize=500`, `HasMoreResults`, `VisibleLimit`), newest-first sort, "Go to last created spell", change-tracking (`HasUnsavedChanges`, `ChangedFields`), onboarding overlay, `ApplyPresetToSelected` command. Build clean across WorldBuilder + Shared + Terminal + Tests. Cherry-pick auto-merged the spell VM cleanly but produced two `[RelayCommand] CopySpell` methods (upstream's new lean form + our 3457ea7 inline-clone form) — resolved by applying `55604d2`'s pattern (see that row). **Dungeon + landscape selection slice landed 2026-04-29 follow-on (now unblocked by `d512ef2`)**: dungeon side — `DungeonEditingContext.SelectedInstancePlacementIndex` + `HasSelectedInstancePlacement`; `InstancePlacementItemViewModel.IsSelected`; `DungeonScene.TryGetWeenieSetupId` accessor; `DungeonObjectRaycast` second pass that tests instance-placement bounds (`IsInstancePlacement` + `InstancePlacementIndex` on the hit), reusing the existing `-50f` dungeon Z bump; `DungeonSelectionManager` gains `InstancePlacementSelectionChangedArgs` + `SelectInstancePlacement` / `DeselectInstancePlacement` / `UpdateInstancePlacementSelectionHighlight` (drives `Scene.SelectedObjectBounds` so the existing selection-box render path lights up the placement) + extended `SyncFromContext` returning a 3-tuple including `placementArgs` + cell/object selectors clear placements via `DeselectInstancePlacement(silent: true)`; `DungeonEditorViewModel` subscribes to the new events, walks `InstancePlacementItems` to flip `IsSelected`, preserves selected index across `RefreshInstancePlacementList`, and decrements/clears the index in `RemoveInstancePlacement` so removals don't dangle; `Dungeon/Tools/SelectTool` raycast click branches on `IsInstancePlacement` (sets context index, clears cell/object) + Escape and box-deselect clear the new index + `Delete` key removes the selected placement, marks the document dirty, refreshes rendering, and writes a status toast. Dungeon hover-resolve hunk skipped (local SelectTool doesn't carry the upstream `Scene.HoveredObjectId/Position/Orientation/Scale` plumbing — would be dead writes). `Dungeon/Views/InstancePlacementsPanelView.axaml` gains the upstream `Classes.selected` styling (selected → `#281640` background + `#d4a843` border + `#f0d080` text), tooltip "Click in the 3D viewport to select. Press Delete to remove selected." Landscape side: `ObjectSelectionState.NotifySelectionChanged` (public raise), `PendingWeenieClassId`, `SelectedOutdoorInstanceIndex` + `HasSelectedOutdoorInstance`; `Deselect` clears the outdoor index. `TerrainEditingContext` constructor gains optional `Project? project` (default null for back-compat) + `Project` get-only + `OutdoorInstancesChanged` event + `NotifyOutdoorInstancesChanged` raiser; `TerrainSystem` passes `project` through. `GameScene._outdoorInstanceRenderObjects` list + `SetOutdoorInstanceRenderObjects` (enqueues each id into the existing `_pendingModelWarmup` queue) + `GetOutdoorInstanceRenderObjects` accessor; `GetAllStaticObjects` always folds outdoor instances into the rendered statics when present. `ObjectRaycast` gains `IsOutdoorInstancePlacement` + `OutdoorInstanceIndex` + a third pass against the new render-objects list (overrides scenery hit so selection doesn't promote to DAT static). `LandscapeEditorViewModel` constructor subscribes to `EditingContext.OutdoorInstancesChanged` and runs `RefreshOutdoorInstanceRenderObjects` (rebuilds the StaticObject list from `Project.OutdoorInstancePlacements` using `_weenieSetupCache`); `OnWeenieSetupsLoaded` also calls it when new setup mappings arrive (so newly resolved placements show up); `DeleteSelectedObject` short-circuits to outdoor-instance removal when one is selected. `SelectSubToolViewModel.UpdateSelectionInfo` adds an "Outdoor Instance | LB X cell N | WCID …" branch; `HandleMouseDown` placement-mode adds a weenie branch (computes outdoor cell index 1-64, appends to `Project.OutdoorInstancePlacements`, calls `Context.NotifyOutdoorInstancesChanged()` so the render list rebuilds); selection branch routes outdoor-instance hits straight to `sel.SelectedOutdoorInstanceIndex` + `NotifySelectionChanged()` without DAT-promoting. **Skipped from this commit (deliberate)**: `ObjectBrowserViewModel` `PlaceWeenieAsInstance` flag + `SelectForPlacement` weenie-vs-static status string + `Views/ObjectBrowserView.axaml` "Place weenie as: Static / DB Instance" radio block — local `ObjectBrowserViewModel` lacks the upstream `ShowWeenies` checkbox + `WeenieClassId` on `ObjectBrowserItem` (those depend on the d512ef2 ObjectBrowser UX shortcut that was deliberately skipped because of structural divergence; see d512ef2 row). Without that gate, `sel.PendingWeenieClassId` is never set, so the weenie placement-click branch is dormant. The AceInstancesPanel from d512ef2 already drives weenie placement through `WeeniePickerViewModel` + `OutdoorInstancePlacements.Add`, so this is just a UX shortcut rather than a hard dependency. Builds clean across WorldBuilder + Shared + Terminal + Tests; 92/93 tests pass (the 1 failure is the pre-existing Linux path-separator test). In-game selection/click-to-delete verification still pending. |
...
| `34c612b` | 04-11 | Defer portal writes via PortalDatDocument; simplify world-gen buildings | 5 | +27/-104 | ✅ PORTED | PortalDatDocument RenderSurface dispatch ✅; `ObjSingleMeshImporter.TrySaveToPortal` deleted (dead code) ✅; `TextureImportService.TryOverwriteUiRenderSurface` defers via PortalDatDocument ✅; LandscapeEditorViewModel world-gen simplify N/A (local already uses simple AddStaticObject pattern); LayoutEditorViewModel `_portalDoc` plumbing ✅ landed 2026-04-30 as part of Wave F (LayoutDetailViewModel ctor takes `PortalDatDocument? portalDoc` and calls `TryOverwriteUiRenderSurface(path, id, _portalDoc)`). |
...
- **5 🟡 PARTIAL** (`b8a09dd` reclassified 2026-04-29 — landscape shader fixes + DocumentManager/LandblockDocument doc-save bug fixes ✅ already silently in place; dungeon-editor refactor majority remains DEFERRED — `d780244` spell-editor UX slice ✅ landed 2026-04-29 + dungeon/landscape selection slice ✅ landed 2026-04-30 (raycast, selection bookkeeping, `Delete` key on selected outdoor + dungeon placements, panel selected-state styling); only the ObjectBrowser-side `PlaceWeenieAsInstance` UX shortcut remains skipped — `d512ef2` re-implemented from intent 2026-04-29 final pass, panels + Project state + DB SQL helpers ✅; ObjectBrowser-side weenie UX shortcut deliberately skipped — see row — `39d68d7` WorldGen substantive tweaks ✅ + FreshStart + big GenerateWorld upgrade ✅ landed 2026-04-30; `ServiceCollectionExtensions` ACME-WorldBuilder-mode bits from the GenerateWorld upgrade not separately needed — `af985b2` reclassified 🟡 2026-04-30: `.github/release.yml` ✅ ported, the two `.github/workflows/*.yml` modifications still 🔧 PERMISSIONS-blocked. `f26345e` reclassified ✅ PORTED 2026-04-30 (Wave F landed); `34c612b` reclassified ✅ PORTED 2026-04-30 (LayoutEditorViewModel `_portalDoc` plumbing landed as part of Wave F))
- **4 🚫 BLOCKED** (`40e9567`, `ee39f71`, `7d6ce84` plus prior; `7d6ce84` blocker rationale changed: Wave F is now landed but our local `LayoutDetailViewModel` shape (no `_stringDats`) still differs from the `7d6ce84` upstream-shape; the `7d6ce84` ElementTreeNode caption/string-resolver pieces are now potentially tractable as a focused follow-up port — see row)
- **13 🔒 DEFERRED** (large refactors or stacked dungeon-chain — port only with in-game testing)
- **0 🔧 PERMISSIONS** (`af985b2` reclassified 🟡 PARTIAL on 2026-04-30 — release-config piece landed, workflow halves still need `workflow`-scope token)
...
### Slice 4 — GUI changes ✅ landed (waves A-G all done)
...
**Wave F** ✅ landed 2026-04-30:
- All four files ported synchronized. Pre-port survey showed local was
  byte-identical to upstream's pre-f26345e baseline for all four —
  the doc's earlier worry about local "100+ LOC of `LayoutEditorViewModel`
  work" was wrong (the ACME-Edition extras `BaseLayoutId/BaseLayoutHex`
  / `Summary` / single-arg `LayoutListItem` ctor are all preserved or
  superset-extended in upstream's f26345e form, so applying the upstream
  diff was a clean port).
- LayoutEditorViewModel.cs (+416 LOC) — gains `LayoutDatDocument` +
  (locally) `PortalDatDocument` document-handles wired in `Init(project)`;
  `Dats` accessor for downstream texture decode; `SaveLayoutToProject`
  [RelayCommand] + `CanSaveLayoutToProject`; `LayoutListItem(uint id,
  bool hasProjectOverride)` ctor adds `*` suffix when stored;
  `OnSelectedLayoutChanged` clones via `LayoutDescBinary.Clone(source,
  id, _dats.Dats.Local)`, picks the project overlay over the DAT when
  present, kicks off `LoadPreviewTexturesAsync`. New `LayoutDetailViewModel`
  with mutable `WorkingLayout`; `Width`/`Height`/`ElementCount` are now
  forwards (`=> WorkingLayout.X`). Edit-fields: `ElementGeomXText/YText/
  WidthText/HeightText/ZText/LeftText/TopText/RightText/BottomText` +
  `ElementReadOrderText` + `PrimarySurfaceHexText` +
  `NewTemplateImageHexText` + `LayoutWidthText/HeightText`. Commands:
  `ApplyLayoutSize`/`ApplyElementGeometry`/`ApplyPrimarySurface`/
  `AddTemplateImage`/`ReplaceUiRenderSurfaceAsync`. Helpers:
  `LoadPreviewTexturesAsync` (background-decodes every node's primary
  surface to a `WriteableBitmap` map at maxEdge=256), `CollectNodes`
  recursive walker, `TryUnpackRenderSurface` defensive unwrap.
  `OnSelectedElementChanged` populates the geom-text fields and the
  primary-surface hex from `LayoutMediaHelper.TryPrimarySurfaceForElement`.
  `ElementTreeNode` is now `partial class … : ObservableObject` with a
  single `Source` property and all geometry exposed as `=> Source.X`
  forwards; `Summary` + `BaseLayoutId/Hex` retained verbatim.
  `SetPosition`/`SetBounds` (clamped to ≥1 width/height), called by
  `LayoutPreviewCanvas` drag-resize. `NotifySourceMutated` raises
  PropertyChanged for every geom field after an external edit.
- LayoutPreviewCanvas.cs (+260 LOC) — gains pointer-input handlers
  (Pressed/Moved/Released/Exited), `PreviewHit` enum (None/Body/N/NE/
  E/SE/S/SW/W/NW), per-handle hit-testing using two pixel thresholds
  (`CornerHitPx=11`, `EdgeHitPx=9`), 8-handle drag-resize math (separate
  branches for each anchor), cursor-on-hover via `CursorForHit`,
  `ClampToLayout` to keep selection within canvas bounds, `_drawScale`/
  `_drawOffset*` cached during `Render`. New optional
  `IReadOnlyDictionary<uint, WriteableBitmap?> elementTextures` arg
  on `SetLayout`; `Render` walks each element and `DrawImage`s the cached
  bitmap before the fill rectangle (try/catch for corrupt bitmaps).
  `DrawResizeHandles` paints 8 `HandleFill`/`HandlePen` `HandleDrawPx=6`
  squares around the selection rect.
- LayoutEditorView.axaml (+188 LOC) — status bar gains `DockPanel` with
  Save-to-project button on the right; properties pane reorganizes into
  Layout-canvas size editor (W/H + Apply), drag-help blurb, and an
  element panel (visible only when SelectedElement != null) with
  Position/Size apply (X/Y/W/H + Z + ReadOrder + Apply geometry button),
  Edges (L/T/R/B), Textures/media (Primary image + Apply, Add template
  + Add, Replace image (in-place)…), Inheritance, UI-states-and-media
  ItemsControl (one bordered card per `LayoutStateRow` from
  `LayoutMediaHelper.PopulateStateRows`), Contents (Children/States
  counts).
- LayoutEditorView.axaml.cs (+85 LOC) — `AttachDetail`/`DetachDetail`/
  `AttachElement`/`DetachElement` event-subscription helpers replace
  the inline-subscribe path; `OnSelectedElementPropertyChanged`
  forwards X/Y/W/H/Z/L/T/R/B/ReadOrder changes into the `Detail` text
  fields via `SyncGeomTextsFromElement`, so dragging a handle in the
  preview live-updates the text boxes (and saves a round-trip when the
  user clicks Apply geometry afterwards). `UpdatePreview` now passes
  `Detail.ElementTextures` to `SetLayout`.
- Local divergence handling: only `TextureImportService.TryOverwriteUiRenderSurface(path, id, portalDoc)`
  diverges from upstream — the local version (from `34c612b`) defers
  writes via `PortalDatDocument`. Threaded `PortalDatDocument` through
  `LayoutDetailViewModel`'s ctor (additional `PortalDatDocument? portalDoc`
  param) and into `_portalDoc`; `CanReplaceUiRenderSurface` now gates
  on `_portalDoc != null`; the call site passes `_portalDoc` as the
  third argument. `LayoutEditorViewModel.Init` resolves the doc via
  `project.DocumentManager.GetOrCreateDocumentAsync<PortalDatDocument>(PortalDatDocument.DocumentId)`.
  `LayoutEditorViewModel` ctor gains optional `TextureImportService?
  textureImport = null` parameter (DI singleton injected automatically).
...
**The big GenerateWorld upgrade — ✅ landed 2026-04-30:**
- All four prereqs now present: `LandblockDocument.ClearAllStatics`
  (added 2026-04-30), `TerrainDocument.ApplyBulkImport` (long-since
  in place from `5faec77`), `DocumentManager.SkipDatStatics` (now has
  real behaviour: `LandblockDocument.InitInternal` early-returns when
  the flag is set, in addition to the existing GameScene reads), and
  `Scene.QueueModelWarmup` (Wave G backfill 2026-04-29). With those
  four landed, the upgrade was straightforward to port:
  `DocumentManager.ResetWorldDocumentsAsync` clears active LB docs
  via `ClearAllStatics` and deletes inactive LB + all dungeon docs
  through the storage service; FreshStart [RelayCommand] +
  `ShowFreshStartConfirmation` dialog (resets terrain to
  WaterDeepSea + clears statics + deletes dungeons); GenerateWorld
  rewritten to pre-sample minimap heights, run Generate inside a
  `Task.Run` with a live progress dialog, set SkipDatStatics during
  the apply phase, queue warmups for every unique
  building/decoration model, and finish with `ShowWorldGenSummary`
  (Settlements list + Export Towns CSV); `ExportTownsCsv`,
  `GetTownTelelocAnchor`, and `BuildAceTeleLoc` helpers. Builds
  clean across Shared + WorldBuilder + Terminal + Tests. FreshStart
  is command-only (no toolbar button) matching upstream.
