...
| `4dc3983` | 03-23 | Weenie property enums + DID/int pickers | many | medium | 🟡 PARTIAL | Cluster B. Dungeon object-browser slice ported 2026-04-29: `DungeonObjectBrowserViewModel.cs` replaced with upstream form (weenies via `AceDbConnector.GetWeenieNamesAsync`, `LoadWeeniesFromDb` command, favorites toggle persisted to `%LOCALAPPDATA%/ACME WorldBuilder/object_browser_favorites.json`, `BatchSize=100`/`Show More` paging, `WeenieSetupsLoaded` event, `IsLoadingWeenies`/`HasMore`/`ShowFavoritesOnly`/`ShowWeenies` observable flags); `DungeonObjectBrowserView.axaml` adds Weenies checkbox + favorites star toggle + "Load Weenies (DB)" button + per-thumbnail star overlay + Show-more button; `ObjectBrowserItem.cs` gains `[ObservableProperty] bool IsFavorite`; `DungeonEditorViewModel` + `CharGenEditorViewModel` callsites pass `Settings:` for DB connection. **Skipped (large, separate slice)**: `AceWeeniePropertyEnums.cs` (873 LOC), `AceWeenieDidPickerModes.cs`, `AceWeenieIntEnumOptions.cs`, `EntityEnums/*` (16 enum files), `AceWeenieTypes.cs` extensions, `WeenieTemplates.json` template additions, `RoomPaletteViewModel`/`SurfaceBrowserViewModel`/`DungeonScene`/`DungeonEditorViewModel`/`DungeonViewportControl` diff hunks (these are the "weenie property pickers + landscape browser wiring" slice — not needed for the dungeon browser feature itself). `AceDbConnector.Weenie.cs +72` already present from Wave C. |
...
### Tally
- **14 ✅ PORTED** (incl. `5faec77` heightmap import + `7293629` mini map landed 2026-04-26; `44f47f8` gizmo improvement and `fa7c58c` surface-browser paging + terrain conformance landed 2026-04-29; `b9ffe3e` dungeon view fill landed 2026-04-29; `15cb9ed` logging + dat-export fix landed 2026-04-29 — closed the audit window; `3457ea7` SpellEditor copy spell + ACE-DB save landed 2026-04-29)
- **5 🟡 PARTIAL** (`f26345e` slices 1-3 ✅ + slice 4 waves A, B, C, D, E, G ✅; wave F (Layout editor) ⏳ — `34c612b` portal-defer + dead-code deletion ✅; LayoutEditorViewModel plumbing pending Wave F — `38b22c2` texture-fix subset ✅, Monster-editor `ReplaceRenderSurfaceAsync` button skipped because Monster editor isn't in this fork — `39d68d7` WorldGen substantive tweaks ✅; FreshStart + big GenerateWorld upgrade skipped, depends on `ClearAllStatics`/`SkipDatStatics`/`QueueModelWarmup` — `4dc3983` dungeon object-browser slice ✅ (weenies/favorites/paging on `DungeonObjectBrowserViewModel`); weenie-property-pickers + landscape-browser wiring + 873-line `AceWeeniePropertyEnums` + 16 EntityEnums skipped, separate slice)
...
- **5 ⏳ TODO** (not yet attempted, lower-risk than the deferred set)
...
| B. Weenie / Spell / Monster editor | ~7 commits, new editor surfaces | Med-High | `3457ea7` SpellEditor copy ✅ ported 2026-04-29. `4dc3983` dungeon object-browser slice 🟡 ported 2026-04-29 (weenies/favorites/paging); the weenie-property-pickers + landscape-browser slice + `AceWeeniePropertyEnums`/EntityEnums remain. Remaining: `d780244`/`55604d2` (spell editor follow-ups; `d780244` likely blocked on `InstancePlacementsPanelView` from `d512ef2`), `239c0c1`/`92fafff`/`d512ef2` (Monster editor + ACE DB instance placements — new editor surfaces). |
...
| F. Misc fixes | Various | Medium | `fa7c58c` + `44f47f8` ported 2026-04-29; `b9ffe3e` ported 2026-04-29; `38b22c2` partial port 2026-04-29 (texture-import safety + docs; Monster GUI button skipped); `39d68d7` WorldGen substantive tweaks (BiomeMapper coastal, Building/RoadGenerator improvements) ✅ ported 2026-04-29; FreshStart + big GenerateWorld upgrade skipped on missing prereqs. |
