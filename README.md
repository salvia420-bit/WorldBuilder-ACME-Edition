...
JSON commands (full schema in `docs/agent_api_reference.md`): `extract-cell-footprints`, `generate-object-sprites`, `render-dungeon`, `emit-tile-pyramid`, `describe-floor`, `emit-static-site`, `emit-render-gallery`, `serve-render-gallery`. The orchestrator chains them; each is independently runnable for inspection or partial regeneration.
