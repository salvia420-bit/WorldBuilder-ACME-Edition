...
JSON commands (full schema in `docs/agent_api_reference.md`): `extract-cell-footprints`, `generate-object-sprites`, `render-dungeon`, `emit-tile-pyramid`, `describe-floor`, `emit-static-site`, `emit-atlas-gallery`, `serve-atlas`. The orchestrator chains them; each is independently runnable for inspection or partial regeneration.
...
The 2026-05-XX wirerender wave adds `emit-atlas-gallery` — a curated Tailwind showcase that auto-picks 5 towns + 5 creature zones + 5 dungeons + 5 region anchors from the gazetteer state, runs `render-preview` + `describe-landblock` per pick, and bundles a single-file viewer (no build step, Tailwind CDN) alongside the renders. The companion `serve-atlas` helper wraps a built-in C# `HttpListener` and detects Tailscale IPs so the gallery streams to any tailnet member via the printed URL. `emit-static-site --gallery` chains the gallery into `<outDir>/gallery/` and cross-links from the Leaflet view's header. Sprite generator's post-66b80ff "buildings only" filter is replaced with a per-mesh bbox flatness check (`min < 0.05 × max` across X/Y/Z), so creature/NPC setups now feed into the atlas instead of falling back to red glyphs — Drudge, Banderling, Mosswart, vendors all render as actual textured top-down sprites under `--use-sprites`.
