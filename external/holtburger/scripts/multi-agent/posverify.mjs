...
while (Date.now() < dl) { const s = await p.evaluate(() => window.__bootState).catch(() => null);
  ...
  if (s === "error") { await p.evaluate(() => { try { window.__sessionHandle.createTestCharacter("Pv" + Date.now().toString().slice(-5)); } catch (e) {} }); await p.waitForTimeout(8000); await p.evaluate(() => { try { window.__runAutonomousLogin({ autoSpawn: "first" }); } catch (e) {} }); }
