...
Grounding: Holtburg building dissection via WB.Terminal + 2026-07-02 live-1070 perf data + code survey 2026-07-28 (anchors below verified against current tree). Companion docs: `~/from-vm/fps-synthesis-20260702.md`, `docs/PLAN-fixed-slot-grid-residency-2026-07-11.md`, `docs/quality-presets.md`.
...
- **Texturing:** hand-written GLSL3 `THREE.ShaderMaterial` (`scene3d/terrain.js:3444`, shader ~lines 1000–1700) over a `sampler2DArray` atlas — 33 layers @ `ATLAS_TILE_PX = 512` (`adapter.js:239`) — already **native** resolution: all 33 retail terrain tiles are 512×512 RGBA8 (empirically dumped 2026-05-20, see `docs/handoff-texture-quality-2026-05-20.md`; `ATLAS_TILE_PX` was bumped 256→512 that day to match). T2's win is therefore added PBR channels + optional 1K sources, not rescuing an upscale. **A real TexMerge port is default-ON** (`readTexMergeFlag` `terrain.js:2974`; Rust half in `crates/holtburger-dat/src/terrain_merge.rs`): per-cell 6 slots (base + 3 terrain overlays + 2 road slots) with retail A8 alpha masks. Fallback path: bilinear 4-corner blend (`terrain.js:1540`). Plus palette LUT tint (`terrain.js:433`), detail textures (~3 slices @256), per-code tiling from Region `tex_tiling`.
