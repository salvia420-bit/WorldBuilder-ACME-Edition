...
- **Wave J3.A — DONE (sequential foundation, 2026-05-27).** PR 7.2 foundation: `<align>` + `<subfield>` codegen support. `crates/holtburger-protocol/build.rs` +609 LOC adding `EmitStep` enum (Field|Align), align emitter mirroring Chorizite's `WriteAlignmentCheck` (`let pad = (N - (*offset % N)) % N`), subfield accessor emission as `pub fn name(&self) -> T` methods (matches Chorizite's `get =>` convention — parent field stays the wire-storage source of truth, subfields are computed). **Key finding (handoff §3 candidate):** Chorizite's `<subfield value=...>` is C# expressions (`parent & MASK`, `parent >> N`, `(uint)1 << ((int)PackedSize >> 24)`, `Size - 16`), NOT the `lowBit`/`bitCount` schema the spec hypothesized. Required a recursive-descent expression translator with parse_or/xor/and/shift/addsub/muldiv/cast_or_unary/atom layers. Schema census: 18 `<align>` sites (13 type-children foundation-tier-reachable, 5 nested inside still-deferred `<switch>` cases), 10 `<subfield>` sites. **Coverage delta:** SKIPPED notes 128 → 118 (-10); generated artifact 23,292 → 23,641 lines (+349); struct count 344 → 354 (+10). New types covered: `ReferralHeader`, `JumpPack`, `PackedMotionCommand`, `CharacterIdentity`, `Action_Movement_AutonomyLevel`, `S2C_Qualities_PrivateUpdateString`, `S2C_Qualities_UpdateString`, `S2C_Item_DeleteObject`, `S2C_Effects_PlayerTeleport`, etc. 6 → 12 generated_parity tests (+6 — covering align math, subfield C#-expr translator, round-trips). 1,737/0 workspace lib tests preserved. Pre-existing `opcode_parity_categorization_gate` integration-test failure on `BookModifyPageResponse 0x00B5` (Chorizite doesn't have it) confirmed unrelated to J3.A; candidate for future allowlist-update follow-on.
