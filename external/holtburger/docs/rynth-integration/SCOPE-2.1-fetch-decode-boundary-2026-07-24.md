...
| **2.2c** | **DONE, but the premise was wrong** — the two `*_OVERRIDE` thread_locals are `#[cfg(test)]` only; on wasm the flags are already shared `OnceLock`s. The REAL defect: `js_location_search()` returns `""` with no `window`, so any worker reads every URL flag at its DEFAULT. **Live bug today** (bake worker ignores `?surfaceCache=off`), race under a pool. Added `seed_url_flag_search()` + `flag_search()` (36 call sites routed), inert until JS calls it. `HM_BATCH_HIST` converted (same diag surface as §2.2b); `ANIM_CACHE` is owner-confined inside an async export — correctly left alone. | **S/M** | ✅ mechanism landed, JS wiring NOT done |
