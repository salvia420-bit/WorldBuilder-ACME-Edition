...
/// `tex_size` field of a `pcode`, at bit 28 (`acclient.c` `GetCellRotation`,
/// `pal_code[n] = ... + (tex_size << 28) + ...`). `tex_size` is `1` whenever
/// `8 / side_cell_count == 1` (full-detail landblock, `side_cell_count == 8`)
/// AND unconditionally `1` on a pal-shifted region, so at the only detail level
/// this renderer emits the bit is always set. It is part of the surface-cache
/// key AND of the PRNG seed ([`texmerge_prng`]) — omitting it makes every
/// alpha-mask draw diverge from retail even when the corners are right.
pub const PCODE_TEX_SIZE_FULL_DETAIL: u32 = 1 << 28;

/// Pack a cell's 4 corner terrain types + 4 corner road codes into a `pcode`,
/// matching acclient's `CLandBlockStruct::GetCellRotation` layout (the
/// `pal_code[0]` term): the 5-bit corner codes sit at bit offsets 15/10/5/0,
/// the 2-bit road fields at offsets 26/24/22/20, and `tex_size` at bit 28.
///
/// **Corners are supplied in acclient's order `[bit15, bit10, bit5, bit0]` =
/// `[SW, SE, NE, NW]`** — `GetCellRotation` reads `terrain[9*ix+iy]` (SW) into
/// bit 15 and `terrain[9*ix+iy+1]` (NW) into bit 0, and `GetTerrain`'s
/// `type[k] = (pcode >> (15 - 5k)) & 0x1F` feeds `BuildTCodes`'s `tcode =
/// 1 << k`, so tcode bit 1 = SW, 2 = SE, 4 = NE, 8 = NW (matching the authored
/// side map's TCode 9 = SW|NW = the WEST edge). `road[k]` (0..3) sits in the
...
pub fn pack_pcode(corners: [u8; 4], road: [u8; 4]) -> u32 {
    let mut p = PCODE_TEX_SIZE_FULL_DETAIL
        | ((corners[0] as u32 & 0x1F) << 15)
...
mod tests {
    ...
    #[test]
    fn pack_pcode_sets_tex_size_bit_28() {
        // GetCellRotation adds `tex_size << 28` to every pal_code, and tex_size
        // is 1 at the only detail level this renderer emits (side_cell_count 8
        // -> 8/8 == 1). The bit feeds texmerge_prng, so dropping it changes the
        // alpha-mask draw on every cell.
        let p = pack_pcode([3, 14, 9, 1], [0, 0, 0, 0]);
        assert_eq!(p & 0xF000_0000, PCODE_TEX_SIZE_FULL_DETAIL);
        // Present on an all-road cell too (road bits live at 20..27, no overlap).
        let pr = pack_pcode([3, 3, 3, 3], [1, 1, 1, 1]);
        assert_eq!(pr & PCODE_TEX_SIZE_FULL_DETAIL, PCODE_TEX_SIZE_FULL_DETAIL);
        assert!(road_code(pr).1, "tex_size bit must not disturb GetRoadCode");
        // Corner codes are unaffected.
        assert_eq!(terrain_codes(p), [3, 14, 9, 1]);
    }

    #[test]
    fn pack_pcode_corner_order_is_sw_se_ne_nw() {
        // acclient bit order: bit15 = terrain[9*ix+iy] = SW, bit0 =
        // terrain[9*ix+iy+1] = NW (GetCellRotation). BuildTCodes' `tcode =
        // 1 << k` over `type[k] = (pcode >> (15-5k)) & 0x1F` therefore means
        // tcode 1 = SW, 2 = SE, 4 = NE, 8 = NW. Pin each single-odd-corner case.
        let t = TexMergeTables::retail();
        for (k, expect_tcode) in [(0usize, 1u32), (1, 2), (2, 4), (3, 8)] {
            let mut corners = [3u8; 4];
            corners[k] = 14; // the odd corner out -> the sole overlay
            let info = texture_merge_info(pack_pcode(corners, [0; 4]), &t).unwrap();
            assert_eq!(info.base_terrain, 3, "corner {k}: base is the repeated type");
            assert_eq!(info.overlays.len(), 1, "corner {k}: exactly one overlay");
            assert_eq!(
                info.overlays[0].tcode, expect_tcode,
                "corner index {k} must carry tcode {expect_tcode}"
            );
        }
        // Adjacent-pair merge: SE+NE share a type -> tcode 2|4 = 6 (east edge).
        // Its mirror (SW+NW) would be 1|8 = 9, the authored side-map TCode --
        // never generated by BuildTCodes, only reached by rotating the 9-map.
        let east = texture_merge_info(pack_pcode([3, 14, 14, 3], [0; 4]), &t).unwrap();
        assert_eq!(east.base_terrain, 3);
        assert_eq!(east.overlays[0].tcode, 6, "SE+NE adjacent pair -> east edge");
    }
