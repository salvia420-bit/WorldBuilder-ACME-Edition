...
mod tests {
    ...
    fn pack_solid(id: u32, surface_type: u32, color: u32, translucency: f32, luminosity: f32, diffuse: f32) -> Vec<u8> {
        let mut buf = Vec::new();
    ...
    fn pack_textured(id: u32, surface_type: u32, tex: u32, pal: u32, translucency: f32, luminosity: f32, diffuse: f32) -> Vec<u8> {
        let mut buf = Vec::new();
        buf.extend_from_slice(&id.to_le_bytes());
        buf.extend_from_slice(&surface_type.to_le_bytes());
    ...
    #[test]
    fn solid_surface_round_trips_color_and_floats() {
        // Base1Solid (0x1) — body is one ARGB colour.
        let buf = pack_solid(0x08001234, 0x01, 0xFF8B6442, 0.0, 0.25, 0.75);
        let surf = Surface::unpack(&buf).unwrap();
        assert_eq!(surf.id, 0x08001234);
    ...
    #[test]
    fn image_surface_reads_texture_and_palette_refs() {
        // Base1Image (0x2) — body is (orig_texture_id, orig_palette_id).
        let buf = pack_textured(0x08005678, 0x02, 0x06001000, 0x04001000, 0.1, 0.2, 0.3);
        let surf = Surface::unpack(&buf).unwrap();
        assert_eq!(surf.id, 0x08005678);
    ...
    fn clipmap_surface_takes_texture_branch() {
        ...
        let buf = pack_textured(0x08009999, 0x04, 0x06002222, 0x04003333, 0.5, 0.5, 0.5);
        let surf = Surface::unpack(&buf).unwrap();
    ...
    fn combined_solid_and_translucent_flags_still_solid() {
        ...
        let buf = pack_solid(0x0800AAAA, 0x11, 0x80FF0000, 0.0, 0.0, 1.0);
        let surf = Surface::unpack(&buf).unwrap();
