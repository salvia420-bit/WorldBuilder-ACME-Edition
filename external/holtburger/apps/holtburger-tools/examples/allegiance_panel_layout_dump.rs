// Dump gmAllegianceUI (LayoutDesc 0x2100002F) so the allegiance plugin
// can confirm element_id → position mapping for the patron/monarch
// header rows, vassals scrollable list, ignore-requests toggle row,
// and Swear / Break / Kick action buttons.
//
// Companion tabs Friends/Squelch share the same layout via
// gmFloatyPanelUI — those are not part of this view yet.
...
const ALLEGIANCE_LAYOUT_ID: u32 = 0x2100002F;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = local.get_file(ALLEGIANCE_LAYOUT_ID)?;
