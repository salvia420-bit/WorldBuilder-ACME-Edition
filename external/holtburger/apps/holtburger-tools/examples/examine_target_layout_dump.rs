// Dump gmFloatyExaminationUI (LayoutDesc 0x2100006B) so the examine-target
// plugin can confirm element_id → position mapping for the outer popup
// frame, header (target name + close), property rows (label/value
// columns), section dividers, and any tab/scroll controls.
...
const EXAMINE_LAYOUT_ID: u32 = 0x2100006B;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = local.get_file(EXAMINE_LAYOUT_ID)?;
