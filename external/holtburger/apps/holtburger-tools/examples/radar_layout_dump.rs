// Dump gmRadarUI (LayoutDesc 0x21000074) so the radar plugin can
// confirm element_id → position mapping for lock button, move handle,
// cardinals, FOV wedge, coords strip, and any other elements.
...
const RADAR_LAYOUT_ID: u32 = 0x21000074;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = local.get_file(RADAR_LAYOUT_ID)?;
