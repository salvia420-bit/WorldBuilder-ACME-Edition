// Dump gmFellowshipUI (LayoutDesc 0x21000030) so the fellowship-panel
// plugin can confirm element_id → position mapping for tabs, member
// list, action row, options, etc.
...
const FELLOWSHIP_LAYOUT_ID: u32 = 0x21000030;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = local.get_file(FELLOWSHIP_LAYOUT_ID)?;
