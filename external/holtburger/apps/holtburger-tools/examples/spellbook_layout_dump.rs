// Dump gmSpellbookUI (LayoutDesc 0x21000032) so the spellbook plugin
// can confirm element_id → position mapping for the school tab strip,
// per-tab buttons, spell grid bounds, individual spell-slot positions,
// detail pane, scrollbar, and search/filter controls.
...
const SPELLBOOK_LAYOUT_ID: u32 = 0x21000032;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = local.get_file(SPELLBOOK_LAYOUT_ID)?;
