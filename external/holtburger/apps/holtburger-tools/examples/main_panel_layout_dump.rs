// Dump gmFloatyPanelUI (LayoutDesc 0x2100006E) so the main-panel plugin
// can confirm element_id → position mapping for the container chrome
// (title bar, close button, body slot, any decorative chrome).
...
const MAIN_PANEL_LAYOUT_ID: u32 = 0x2100006E;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = local.get_file(MAIN_PANEL_LAYOUT_ID)?;
