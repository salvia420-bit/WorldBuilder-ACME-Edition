// Dump gmContractsUI (LayoutDesc 0x21000069) so the contracts-panel plugin
// can confirm element_id → position mapping for the tab strip, contract
// list / grid bounds, per-row positions, detail pane, action buttons,
// and any chrome (counter / cap labels, dividers).
...
const CONTRACTS_LAYOUT_ID: u32 = 0x21000069;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = local.get_file(CONTRACTS_LAYOUT_ID)?;
