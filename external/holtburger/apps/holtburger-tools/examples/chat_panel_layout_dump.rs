// Dump gmFloatyMainChatUI (LayoutDesc 0x2100006F) so the chat-panel
// plugin can confirm element_id → position mapping for the chat-log
// container, text area, scrollbar, 4 left-edge filter buttons,
// top-right maximize button, input row, channel selector, text input
// field, and send button.
//
// This is the chat-panel-only counterpart to `chat_layout_dump.rs`
// (which also dumps 0x21000070 gmFloatyToolbarUI). Scoped narrowly so
// the layout-port verification + plan-doc references stay focused.
...
const CHAT_LAYOUT_ID: u32 = 0x2100006F;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = local.get_file(CHAT_LAYOUT_ID)?;
