// Dump gmFloatyMainChatUI (LayoutDesc 0x2100006F).
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for id in [0x2100006Fu32, 0x21000070u32] {
...
fn dump_el(el: &holtburger_dat::file_type::ElementDesc, depth: usize) {
    ...
    let st = if el.states.is_empty() { String::new() } else { format!(" states={}", el.states.len()) };
    println!(
        ...
        el.default_state, el.read_order, st,
        el.children.len(),
    );
