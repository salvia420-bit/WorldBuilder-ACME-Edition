// Dump gmMapUI (LayoutDesc 0x21000026) so the map-panel plugin can
// confirm element_id → position mapping for world bitmap area, zoom
// controls, coord readout, panel chrome, and any scroll-pan controls.
...
const MAP_LAYOUT_ID: u32 = 0x21000026;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = local.get_file(MAP_LAYOUT_ID)?;
