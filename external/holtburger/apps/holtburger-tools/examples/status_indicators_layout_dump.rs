// Dump gmFloatyIndicatorsUI (LayoutDesc 0x21000071) so the status-indicators
// plugin can confirm element_id → position mapping for each status icon.
...
const STATUS_LAYOUT_ID: u32 = 0x21000071;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = local.get_file(STATUS_LAYOUT_ID)?;
