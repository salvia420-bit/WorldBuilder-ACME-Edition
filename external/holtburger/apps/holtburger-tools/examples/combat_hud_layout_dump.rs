// Dump gmCombatUI (LayoutDesc 0x21000007) so the combat-hud plugin can
// confirm element_id → position mapping for stance/power-bar/attack-height
// triangle (Hi/Mid/Lo) and any chrome elements.
...
const COMBAT_HUD_LAYOUT_ID: u32 = 0x21000007;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = local.get_file(COMBAT_HUD_LAYOUT_ID)?;
