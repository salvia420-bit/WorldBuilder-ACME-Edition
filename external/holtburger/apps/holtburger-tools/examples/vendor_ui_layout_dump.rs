// Dump gmVendorUI (LayoutDesc 0x21000012) so the vendor-ui plugin can
// confirm element_id → position mapping for tabs, category dropdown,
// icon strip, selected-item name/price, action buttons (Buy / Add /
// Clear / Confirm), close button, and any other wirable elements.
...
const VENDOR_LAYOUT_ID: u32 = 0x21000012;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = local.get_file(VENDOR_LAYOUT_ID)?;
