...
//   [H] Per-plugin manifest.json files — all 30 bundled manifests
//       parse + validate
//
...
(async () => {
  ...
  // ─── [H] Per-plugin manifest.json files — schema parse for all 30 ───
  ...
  check('index.json is well-formed and lists 30 plugins', () => {
    const txt = fs.readFileSync(INDEX_PATH, 'utf8');
    const j = JSON.parse(txt);
    assert.ok(Array.isArray(j.plugins), 'plugins must be array');
    assert.equal(j.plugins.length, 30, `expected 30, got ${j.plugins.length}`);
  });
  ...
  check(`found ${manifestFiles.length} manifest files (expected 30)`, () => {
    assert.equal(manifestFiles.length, 30);
  });
  ...
  check('all 30 manifests resolve as a single dependency graph (cycle-free)', () => {
    ...
    assert.equal(started.length, 30, `expected 30 started, got ${started.length}: ${started}`);
