...
//   [H] Per-plugin manifest.json files — all 29 bundled manifests
//       parse + validate
//
...
(async () => {
  ...
  // ─── [H] Per-plugin manifest.json files — schema parse for all 29 ───
  ...
  check('index.json is well-formed and lists 29 plugins', () => {
    const txt = fs.readFileSync(INDEX_PATH, 'utf8');
    const j = JSON.parse(txt);
    assert.ok(Array.isArray(j.plugins), 'plugins must be array');
    assert.equal(j.plugins.length, 29, `expected 29, got ${j.plugins.length}`);
  });
  ...
  check(`found ${manifestFiles.length} manifest files (expected 29)`, () => {
    assert.equal(manifestFiles.length, 29);
  });
  ...
  check('all 29 manifests resolve as a single dependency graph (cycle-free)', () => {
    ...
    assert.equal(started.length, 29, `expected 29 started, got ${started.length}: ${started}`);
