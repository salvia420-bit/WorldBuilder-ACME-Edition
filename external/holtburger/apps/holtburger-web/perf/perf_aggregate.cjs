...
function rankByLandblock(samples, opts) {
  ...
  var ranked = Object.keys(bins).map(function (lb) {
    ...
    return {
      ...
      p95_worst: round(b.worsts.length ? Math.max.apply(null, b.worsts) : median(b.p95s), 1),
  ...
  ranked.sort(function (a, b) {
    ...
    if (b.p95_worst !== a.p95_worst) return b.p95_worst - a.p95_worst;
...
function renderLeagueMarkdown(ranked, meta) {
  meta = meta || {};
  var out = [];
  out.push("# perf league — worst content by frame-time p95");
  out.push("");
  ...
  out.push("");
  out.push("| # | landblock | p95 ms (med) | p95 worst | draws/f | tris/f | heap MB | dwell s | samples |");
  out.push("|--:|:--|--:|--:|--:|--:|--:|--:|--:|");
  ranked.forEach(function (r, i) {
    out.push("| " + (i + 1) + " | `" + r.lb + "` | " + r.p95_med + " | " + r.p95_worst + " | " +
      (r.draw_med == null ? "—" : r.draw_med) + " | " + (r.tri_med == null ? "—" : r.tri_med) + " | " +
      (r.heap_med == null ? "—" : r.heap_med) + " | " + r.dwellSec + " | " + r.samples + " |");
  });
  out.push("");
  return out.join("\n");
}
...
function sliceTourLbs(ranked, opts) {
  ...
  var healthy = ranked.slice().sort(function (a, b) { return a.p95_med - b.p95_med; })
