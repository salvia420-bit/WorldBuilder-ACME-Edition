...
- **2026-05-18 — Direct render path triaged (kept).** The path is reachable during the Bruneton bake window (~8 s on GTX 1070, ~43 s on R9 290), under `?atmosphere=off`, and as an atmosphere-init-failure fallback. Without it, users would see a blank screen for tens of seconds every login. Cloud-paint-over-geometry was the historic concern but is closed: cloud overlay attaches to `skyDome.skyScene` so direct path's `renderSkyPass → clearDepth → renderer.render(scene)` does the same render-order occlusion as the atmosphere composer. Updated comments in `scene3d/index.js` to explain the reachability cases.
- **2026-05-18 — Diagnostic logs removed.** Dropped `_visDiagCount` from `cells.js` (never fired — `liveScene3d.sessionHandle` is never set), `[clouds/diag frame=1/60]` + `[clouds/diag renderOverlay first call]` + `_depthWireWarned` from `cloud_overlay.js`. All were one-shot or watchdog logs added during the AMD R9 290 depth-discard investigation, no longer load-bearing now that clouds use render-order. `setDepthDiscardEnabled()` stays as a feature API.
