...
- 3. flip rest default-on (SPLIT per-flag for bisectability):
  - [x] **3a. unifiedClone default-on** + doc-reconcile unifiedClientEvent/singleDriver (already on in code). `index.html:4638` → `v?.toLowerCase() !== "off"`. Verify: logic; dedicated `test_a15_q2_entity_update_clone.mjs` **21/0**; Tier-1 baseline-identical; browser smoke GREEN (clean connect → in-world 19.6s, 0 console errors). Commit: `<pending>`.
