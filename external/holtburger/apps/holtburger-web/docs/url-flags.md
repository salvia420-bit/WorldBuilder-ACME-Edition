...
| **`?mtQueue=on`** | **A4-Q2 (2026-06-12, W3+ S5)** — wires one-shot overlay COMPLETION across the wasm boundary into the A4-Q1 `MotionTableManager` queue: tagged (`mtQueued`-played via `_tryPlayLink`) LOCAL-player overlays call `window.__notifyAnimationDone(guid, true)` (index.html bridge next to the SessionHandle) → wasm `SessionHandle.notifyAnimationDone` → cmd channel → recv-arm local-guid filter → `MovementSystemHandle::notify_animation_done` → `MotionTableManager::animation_done` (retail `AnimDoneHook::Execute` → `Hook_AnimDone` → `CPartArray::AnimationDone` → `MotionTableManager::AnimationDone`, acclient.c:342336 → :317087 → :325080 → :329873; success hard-coded 1 on the renderer path, :317093). Eviction of a tagged, not-yet-completed overlay notifies `success=false` (hang prevention; exit-world-drain analogy, acclient.c:329940-329947). Counter-poisoning guards: per-instance `_mtQueuedKeys` tagging contract (only pipeline-queued overlays notify — NO current caller tags; enqueue sources arrive with Stage-2 `?interpRig`/A3-D2), local-guid filter on BOTH sides. Independently flippable from `?hookDrain`: with `?hookDrain=off` a mixer `finished` listener at the tagged play site is the fallback completion detector; full retail ORDERING parity (completion AFTER the final frame's hooks) only with BOTH on. Rust side inert unless `USE_MOTION_TABLE_QUEUE` (core const, §6), and empty-queue no-op even then (acclient.c:329884 head-null guard). `WASM_EXPORT_MANIFEST_VERSION` → **v4** (index.html EXPECTED stays 1; the caller is flag-gated + typeof-guarded so a v3 pkg soft-degrades). Needs a wasm rebuild. Default OFF = zero notify calls, byte-identical. **Pending 1070 eye-test** (gated further on A3-D2 enqueue sources + `?interpRig`). | (Requires A3-D2 + `?interpRig` + `USE_MOTION_TABLE_QUEUE` + both flags on) spam-click swing; emote then move; one-shot completion | Spam-click swing shows queue truncation (no crossfade churn); emote completes then gait resumes; one-shot completion releases action state; flag off = byte-identical. |
