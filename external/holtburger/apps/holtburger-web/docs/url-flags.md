...
| `surfaceUnified` | `on`/`1`/`true` | **off** | A10-M1 (2026-06-11): route BOTH Surface (0x08) flag→material decode sites through the single `applySurfaceRenderState` (the JS analogue of retail `D3DPolyRender::SetSurface`). The cache path is byte-identical (proven: 70/70 flag×float combos in `test_f7_8_surface_bitfield.mjs`); the dyed/paletted path additionally attaches the luminous `emissiveMap` so dyed luminous gear stops washing to white (resolves the contradictory-comment split, A10 §3 row 2 / ROADMAP §7 item 2). JS-live. Off = legacy dual-path. 1070-gated: dyed-luminous eye-test (lifestone, dyed glow gear). | scene3d/materials.js `applySurfaceRenderState` / `readSurfaceUnifiedFlag`; entities.js `_applyPalettedSurfaceRenderState` |
