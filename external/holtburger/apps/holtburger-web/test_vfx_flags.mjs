...
check("VFX_EFFECT_FLAGS maps all 9 effect ids (Phase-1 + tipFlex + particle.gemSparkle)", ALL_IDS.length === 9, ALL_IDS.join());
...
check("?visual=all: all 9 effects active", vfxActiveEffectIds().length === 9);
...
check("?visual=all&glint=off: 8 effects active", vfxActiveEffectIds().length === 8);
