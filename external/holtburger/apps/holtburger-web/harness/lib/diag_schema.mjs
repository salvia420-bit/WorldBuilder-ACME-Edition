...
export const REGISTRY = Object.freeze([
...

  {
    ...
    fields: {
      ...
      "relief.variantRowsResident": L("count", null, { note: "GEOMR rows in the RESIDENT packs — a LIVE accessor over the wasm export `geom_relief_rows_resident()`, read at read time (2026-08-13). It used to be a one-shot sample taken inside initGeomBundles, which runs BEFORE any pack is resident: on a cold boot it latched 0 and the derived \"baked without --geom-relief\" warning cried wolf against a dist carrying 2,179 GEOMR rows (1070 relief eye-shot, 2026-08-13 — the live export read 128 moments later). Read it directly; the T4/GFXOBJ-RELIEF caveat \"read the wasm export, not the diag field\" is RETIRED. 0 while nothing is resident is normal; 0 AFTER models assembled is the bad-bake tell, and that is the condition the deferred warning now uses." }),
      "relief.modelsAssembled": C("count"),
      "chain.evaluated": L("bool", null, { note: "ARMING-CHAIN (2026-08-13): false until initGeomBundles has run (install-on-arm)" }),
      "chain.armed": L("bool", null, { note: "the geomBundles path armed" }),
      "chain.reliefArmed": L("bool", null, { note: "the relief VARIANT arm engaged (all four legs held)" }),
      "chain.firstUnmetLeg": L("string", null, { note: "name of the FIRST false leg, or null when everything held — ends the 'which leg disarmed it' guessing game that previously required capturing a console.error" }),
      "chain.legs": L("json", null, { note: "ordered truth table: geomBundlesFlag, packSourceFlag, placementId, packController, gfxReliefOn, reliefBundlesFlag, reliefWasmExport, gfxSubdivLevel0, wasmAssembleExports" }),
      "chain.reasons": L("json", null, { note: "the same human-readable disarm reasons the console.error prints" }),
    },
  },
