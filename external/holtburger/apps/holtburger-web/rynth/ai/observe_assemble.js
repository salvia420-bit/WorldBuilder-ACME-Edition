...
function pub(s) {
  return { subsystem: s.subsystem, tier: s.tier, id: s.id, tokens: s.tokens };
}

/**
 * assembleObservation(sections, { totalTokens, quotas, separator })
 *
 *  sections: Array<{ subsystem?, tier?, text, id? }> in INJECTION order (that
 *            order is preserved in the output; tiers govern only what is SHED).
...
export function assembleObservation(sections, opts = {}) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    norm.push({ subsystem, tier, text, id, index: i, tokens: estimateTokens(text) });
  }
  ...
  // 3) Total budget: fill by KEEP priority (tier rank asc, then injection
  //    order) so the most salient sections claim the budget first; the least
  //    salient (STEADY, last) get the remainder — truncated or shed. This is
  //    what makes "drop STEADY first" fall out for free.
  ...
  //    slice); in the common under-budget case nothing is cut here.
  const kept = alive.filter((s) => keep.has(s));
  let text = kept.map((s) => s.text).join(sep);
  if (estimateTokens(text) > totalTokens) text = text.slice(0, tokensToChars(totalTokens));
