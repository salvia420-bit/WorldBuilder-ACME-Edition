...
namespace WorldBuilder.Editors.Landscape.ViewModels {
    public partial class HeightSetSubToolViewModel : SubToolViewModelBase {
        ...
        private void ApplyPreviewChanges(Vector3 centerPosition) {
            // Delegate vertex lookup and set-height computation to shared algorithms
            var affected = TerrainAlgorithms.GetAffectedVertices(centerPosition, BrushRadius,
                (x, y) => Context.GetHeightAtPosition(x, y));

            var setChanges = TerrainAlgorithms.ComputeSetHeight(affected, TargetHeight,
                lbId => Context.TerrainSystem.GetLandblockTerrain(lbId));

            // Apply changes, skipping already-pending vertices
            var batchChanges = new Dictionary<ushort, Dictionary<byte, uint>>();

            foreach (var (lbId, vIndex, original, newHeight) in setChanges) {
