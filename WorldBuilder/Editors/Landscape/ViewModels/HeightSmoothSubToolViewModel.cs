...
namespace WorldBuilder.Editors.Landscape.ViewModels {
    public partial class HeightSmoothSubToolViewModel : SubToolViewModelBase {
        ...
        private void ApplyPreviewChanges(Vector3 centerPosition) {
            // Delegate vertex lookup and smoothing math to shared algorithms
            var affected = TerrainAlgorithms.GetAffectedVertices(centerPosition, BrushRadius,
                (x, y) => Context.GetHeightAtPosition(x, y));

            var smoothChanges = TerrainAlgorithms.ComputeSmooth(affected, Strength,
            ...
            foreach (var (lbId, vIndex, original, newHeight) in smoothChanges) {
