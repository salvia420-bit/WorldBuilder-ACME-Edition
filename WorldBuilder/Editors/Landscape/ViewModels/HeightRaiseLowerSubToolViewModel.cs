...
namespace WorldBuilder.Editors.Landscape.ViewModels {
    public partial class HeightRaiseLowerSubToolViewModel : SubToolViewModelBase {
        ...
        private void ApplyPreviewChanges(Vector3 centerPosition) {
            // Delegate vertex lookup and height computation to shared algorithms
            var affected = TerrainAlgorithms.GetAffectedVertices(centerPosition, BrushRadius,
                (x, y) => Context.GetHeightAtPosition(x, y));

            int delta = _isLowering ? -Strength : Strength;

            // Build a combined processed set for the shared algorithm
            var alreadyProcessed = new HashSet<(ushort, int)>();
            foreach (var (lbId, processed) in _processedVertices) {
                foreach (var vIdx in processed) alreadyProcessed.Add((lbId, vIdx));
            }

            var heightChanges = TerrainAlgorithms.ComputeRaiseLower(affected, delta,
                lbId => Context.TerrainSystem.GetLandblockTerrain(lbId),
                alreadyProcessed);

            // Apply changes
            var batchChanges = new Dictionary<ushort, Dictionary<byte, uint>>();

            foreach (var (lbId, vIndex, original, newHeight) in heightChanges) {
                // Track as processed
                if (!_processedVertices.TryGetValue(lbId, out var processed)) {
                    processed = new HashSet<int>();
                    _processedVertices[lbId] = processed;
                }
                processed.Add(vIndex);
