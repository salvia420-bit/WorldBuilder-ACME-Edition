...
namespace WorldBuilder.Shared.Documents {
    ...
    public partial class LandblockDocument : BaseDocument {
        ...
        protected override Task<bool> SaveToDatsInternal(IDatReaderWriter datwriter, int iteration = 0) {
            ...
            foreach (var building in lbi.Buildings) {
                ...
                for (int i = 0; i < _data.StaticObjects.Count; i++) {
                    ...
                    // an existing destination building's interior/cell graph).
                    var candidateLocal = ReverseOffset(obj.Origin, lbId);
                    if (BuildingBlueprintCache.TryGetPlacementDonorHint(
                            obj.Id, (ushort)lbId, candidateLocal, out var donorLbKey, out _) &&
                        donorLbKey != (ushort)lbId) {
                        continue;
                    }
            ...
            for (int i = 0; i < _data.StaticObjects.Count; i++) {
                ...
                if (BuildingBlueprintCache.IsBuildingModelId(obj.Id, datwriter)) {
                    var blueprint = BuildingBlueprintCache.GetBlueprintForPlacement(
                        obj.Id, (ushort)lbId, localOrigin, datwriter, _logger);
                    if (blueprint != null) {
            ...
            // Keep LandBlock.HasObjects aligned with whether LandBlockInfo actually has content.
            // If this flag is false, the client may ignore exterior objects/buildings even when
            // a LandBlockInfo entry exists (common on destination landblocks that originally had no LBI).
            if (datwriter.TryGet<LandBlock>(landId, out var landblock)) {
                bool shouldHaveObjects = lbi.Objects.Count > 0 || lbi.Buildings.Count > 0;
                if (landblock.HasObjects != shouldHaveObjects) {
                    landblock.HasObjects = shouldHaveObjects;
                    if (!datwriter.TrySave(landblock, iteration)) {
                        _logger.LogError("[LBDoc]   FAILED to save LandBlock 0x{LandId:X8} (HasObjects={HasObjects})",
                            landId, shouldHaveObjects);
                        return Task.FromResult(false);
                    }
                    _logger.LogInformation("[LBDoc]   Updated LandBlock 0x{LandId:X8} HasObjects={HasObjects}",
                        landId, shouldHaveObjects);
                }
            }
            else {
                _logger.LogWarning("[LBDoc]   Could not load LandBlock 0x{LandId:X8} to sync HasObjects flag", landId);
            }
        ...
        /// <summary>
        /// Removes all static objects from this landblock.
        /// Returns the number of objects that were removed.
        /// </summary>
        public int ClearStaticObjects() {
            int count = _data.StaticObjects.Count;
            if (count == 0) return 0;
