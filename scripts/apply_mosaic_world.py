#!/usr/bin/env python3
"""
apply_mosaic_world.py — Apply mosaic heightmaps via WorldBuilder.Terminal

Pipes JSON commands through the terminal's --stdin mode to:
  1. Load the project
  2. Apply all heightmaps from data/mosaic_heightmaps.jsonl
  3. Apply terrain types (retail painting per mosaic piece)
  4. Export to DAT files

Usage:
    .venv311\\Scripts\\python.exe scripts\\apply_mosaic_world.py [--skip-export]
"""
...
PROJECT_FILE = PROJECT_ROOT / "projects" / "TestProject" / "TestProject.wbproj"
...
def main():
    ...
    print("=" * 70)
    print("  APPLY MOSAIC WORLD -- WorldBuilder.Terminal Pipeline")
    print("=" * 70)
    ...
    with open(HEIGHTMAP_FILE, "r", encoding="utf-8") as f:
        for line_num, line in enumerate(f, 1):
            ...
            # Apply terrain types (retail painting carried with each mosaic piece)
    ...
    # Step 3: Skip auto-paint (retail terrain types applied above as part of mosaic)
    print(f"\n  Skipping auto-paint (using retail terrain types from mosaic pieces)")

    # Step 4: Export (unless --skip-export)
    if not skip_export:
        export_dir = str(PROJECT_ROOT / "projects" / "TestProject" / "output")
    ...
    print(f"\n{'='*70}")
    print(f"  MOSAIC WORLD APPLIED in {total_time:.0f}s ({total_time/60:.1f} min)")
