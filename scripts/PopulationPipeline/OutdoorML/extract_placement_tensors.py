...
from settlement_signatures import service_targets_for_signature
from settlement_signatures import settlement_role_one_hot, SERVICE_TARGET_LABELS, SETTLEMENT_ROLE_LABELS, settlement_signature_weight
...
CONTEXT_DIM = BASE_CONTEXT_DIM + len(SETTLEMENT_ROLE_LABELS) + len(SERVICE_TARGET_LABELS)
...
def build_context_vector(lb_x: int, lb_y: int,
                         ...
                         service_targets: Optional[list[float]] = None) -> np.ndarray:
    """
    ...
    Layout:
      ...
      [229:232] = soft service targets (portal, lifestone, vendor)
    """
    ...
    role_end = BASE_CONTEXT_DIM + len(SETTLEMENT_ROLE_LABELS)
    ctx[BASE_CONTEXT_DIM:role_end] = np.array(settlement_role_one_hot(role), dtype=np.float32)
    if service_targets is None:
        service_targets = [0.0] * len(SERVICE_TARGET_LABELS)
    ctx[role_end:CONTEXT_DIM] = np.array(service_targets, dtype=np.float32)
    
    return ctx


def load_wcid_names(sql_path: str) -> Dict[int, str]:
    """Parse the weenie table from the SQL dump to build wcid -> class name."""
    if not sql_path or not os.path.exists(sql_path):
        return {}

    print("  Loading weenie names from SQL dump...")
    wcid_names: Dict[int, str] = {}
    row_re = re.compile(r"\((\d+),'((?:''|[^'])*)',(\d+),'[^']*'\)")
    ...
    with open(sql_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            for match in row_re.finditer(line):
                ...
                wcid_names[int(match.group(1))] = name

    print(f"    Loaded {len(wcid_names)} wcid->name rows")
    return wcid_names


def classify_slumlord_house_type_from_name(name: Optional[str]) -> Optional[str]:
    """Fallback classifier for slumlord WCIDs not covered by explicit retail WCID families."""
    if not name:
        return None

    lowered = name.lower()
    if 'mansion' in lowered or 'manor' in lowered or 'estate' in lowered:
        return 'Mansion'
    if 'villa' in lowered:
        return 'Villa'
    if 'cottage' in lowered or 'bungalow' in lowered:
        return 'Cottage'
    return None
...
def build_training_examples(instances_by_lb, links, wcid_to_idx, wcid_types,
    ...
    for idx, lb_key in enumerate(populated_lbs):
        ...
        service_targets = service_targets_for_signature(settlement_signature, family_labels)
        ...
        for obj_idx in range(n):
            ...
            housing_token, housing_type = classify_housing_token(wcid, wtype, wcid_names)
