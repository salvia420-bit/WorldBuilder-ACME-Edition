...
def compute_dense_service_composition_weights(labels: np.ndarray, dense_labels: list[str]) -> np.ndarray:
    counts = np.bincount(labels, minlength=len(dense_labels)).astype(np.float32)
    weights = np.ones(len(dense_labels), dtype=np.float32)
    label_to_idx = {label: idx for idx, label in enumerate(dense_labels)}

    if 'none' in label_to_idx:
        weights[label_to_idx['none']] = 0.35
    for label in (
        ...
        if idx is None or counts[idx] <= 0:
            continue
        weights[idx] = min(max(24.0 / counts[idx], 1.5), 4.0)
...
def main() -> None:
    ...
    dense_service_composition_weights_np = compute_dense_service_composition_weights(
        dense_service_compositions[train_idx],
        meta.get("dense_service_composition_labels", []),
    )
    archetype_weights = torch.from_numpy(archetype_weights_np).float().to(device)
    dense_service_composition_weights = torch.from_numpy(dense_service_composition_weights_np).float().to(device)
    ...
    if len(meta.get("dense_service_composition_labels", [])) > 0:
        print("  Dense service composition weights:")
        for label, weight in zip(meta["dense_service_composition_labels"], dense_service_composition_weights_np.tolist()):
            print(f"    {label:24s} {weight:.2f}")
    ...
    for epoch in range(args.epochs):
        ...
        for ctx, arch, style, dense_comp, fam in train_loader:
            ...
            loss_dense_comp = (
                F.cross_entropy(dense_comp_logits, dense_comp, weight=dense_service_composition_weights)
            ...
            loss = loss_arch + 0.35 * loss_style + 0.15 * loss_dense_comp + 0.6 * loss_fam
