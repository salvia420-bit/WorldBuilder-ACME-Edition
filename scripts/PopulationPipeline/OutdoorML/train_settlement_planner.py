...
class PlannerDataset(Dataset):
    def __init__(self, contexts: np.ndarray, archetypes: np.ndarray, service_styles: np.ndarray,
                 service_motifs: np.ndarray, family_bins: np.ndarray):
        ...
        self.service_motifs = torch.from_numpy(service_motifs).long()
    ...
    def __getitem__(self, idx: int):
        return self.contexts[idx], self.archetypes[idx], self.service_styles[idx], self.service_motifs[idx], self.family_bins[idx]


class SettlementPlanner(nn.Module):
    def __init__(self, context_dim: int, archetype_classes: int, family_heads: int,
                 service_style_classes: int = 0, service_motif_classes: int = 0):
        super().__init__()
        ...
        self.service_motif_head = nn.Linear(256, service_motif_classes) if service_motif_classes > 0 else None
    ...
    def forward(self, ctx: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor | None, torch.Tensor | None, torch.Tensor]:
        ...
        service_motif_logits = self.service_motif_head(hidden) if self.service_motif_head is not None else None
        family_logits = self.family_head(hidden).view(ctx.shape[0], self.family_heads, 4)
        return archetype_logits, service_style_logits, service_motif_logits, family_logits
...
def compute_metrics(
    ...
    service_motif_logits: torch.Tensor | None,
    service_motifs: torch.Tensor,
    ...
    service_motif_acc = 0.0
    if service_motif_logits is not None:
        service_motif_acc = (service_motif_logits.argmax(dim=-1) == service_motifs).float().mean().item()
    ...
    return {
        ...
        "service_motif_acc": service_motif_acc,
...
def main() -> None:
    ...
    service_motifs = data["service_motifs"]
    ...
    train_ds = PlannerDataset(
        contexts[train_idx], archetypes[train_idx], service_styles[train_idx], service_motifs[train_idx], family_bins[train_idx]
    )
    val_ds = PlannerDataset(
        contexts[val_idx], archetypes[val_idx], service_styles[val_idx], service_motifs[val_idx], family_bins[val_idx]
    )
    ...
    model = SettlementPlanner(
        ...
        service_motif_classes=len(meta.get("service_motif_labels", [])),
    ).to(device)
    ...
    for epoch in range(args.epochs):
        model.train()
        train_loss = 0.0
        for ctx, arch, style, motif, fam in train_loader:
            ctx = ctx.to(device)
            arch = arch.to(device)
            ...
            arch_logits, style_logits, motif_logits, fam_logits = model(ctx)
            ...
            loss_motif = (
                F.cross_entropy(motif_logits, motif)
                if motif_logits is not None else torch.tensor(0.0, device=device)
            )
            loss_fam = F.cross_entropy(fam_logits.reshape(-1, 4), fam.reshape(-1))
            loss = loss_arch + 0.35 * loss_style + 0.25 * loss_motif + 0.6 * loss_fam
        ...
        model.eval()
        val_loss = 0.0
        val_metrics = {"archetype_acc": 0.0, "service_style_acc": 0.0, "service_motif_acc": 0.0, "family_acc": 0.0}
        val_batches = 0
        with torch.no_grad():
            for ctx, arch, style, motif, fam in val_loader:
                ctx = ctx.to(device)
                arch = arch.to(device)
                style = style.to(device)
                motif = motif.to(device)
                ...
                loss = loss_arch + 0.35 * loss_style + 0.25 * loss_motif + 0.6 * loss_fam
                val_loss += loss.item()
                batch_metrics = compute_metrics(arch_logits, arch, style_logits, style, motif_logits, motif, fam_logits, fam)
        ...
        if avg_val < best_val:
            ...
            best_state = {
                ...
                "service_motif_labels": meta.get("service_motif_labels", []),
        ...
        print(
            ...
            f"motif_acc={val_metrics['service_motif_acc']:.3f} "
