...
DEFAULT_RESUME="$MODEL_DIR/world_grammar_town_component_linked_abstract_ace_20260405T040702Z_start_resume.pt"
DEFAULT_EPOCHS=500
...
printf '%s\n' \
  "python3 -u scripts/PopulationPipeline/WorldGrammar/train_town_grammar.py --resume $RESUME_PATH --tensor-path $TENSOR_PATH --vocab-path $VOCAB_PATH --run-name $RUN_NAME --epochs $EPOCHS --checkpoint-every 25 --resume-checkpoint-every 10" \
  > "$RUN_DIR/command.txt"
printf '%s\n' "$SESSION_NAME" > "$RUN_DIR/session.txt"
printf '%s\n' "$LOG_PATH" > "$RUN_DIR/log_path.txt"

TMUX_CMD="cd $(printf '%q' "$ROOT") && exec -a world-grammar-town-resume python3 -u scripts/PopulationPipeline/WorldGrammar/train_town_grammar.py --resume $(printf '%q' "$RESUME_PATH") --tensor-path $(printf '%q' "$TENSOR_PATH") --vocab-path $(printf '%q' "$VOCAB_PATH") --run-name $(printf '%q' "$RUN_NAME") --epochs $(printf '%q' "$EPOCHS") --checkpoint-every 25 --resume-checkpoint-every 10 > $(printf '%q' "$LOG_PATH") 2>&1"

tmux new-session -d -s "$SESSION_NAME" "$TMUX_CMD"
sleep 1
tmux list-panes -t "$SESSION_NAME" -F '#{pane_pid}' > "$RUN_DIR/pid.txt"

echo "Launched detached town-grammar resume run."
