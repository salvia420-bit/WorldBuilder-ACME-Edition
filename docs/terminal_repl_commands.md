...
- `emit-render-gallery <outDir> [--towns N] [--zones N] [--dungeons N] [--regions N] [--radius N] [--res N] [--no-sprites] [--no-overlay]` — auto-curates a 20-pick gallery (5 towns + 5 creature zones + 5 dungeons + 5 region anchors by default) from the project's gazetteer state, renders each pick via `render-preview`, and bundles a Tailwind single-file viewer into `<outDir>` with `describe-landblock` text loading as the side panel.
- `serve-render-gallery <outDir> [--port N] [--bind addr]` — serves the gallery directory over HTTP using a built-in `HttpListener`. Prints the local URL plus a Tailscale URL when the host is on a tailnet. Listener runs in the background until the terminal exits.
