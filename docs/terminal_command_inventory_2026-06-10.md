...
| `render-surface-import` | `datPath, idHex, pngPath, format?, allowResize?, dryRun?, allowCreate?` \| `imports[{idHex,pngPath,format?,allowResize?}]` \| `fromDir` | ✓ | `CmdRenderSurfaceImport` | Import a PNG into a portal-DAT COPY as a RenderSurface with the record's own PixelFormat **preserved** by default (DXT1 stays DXT1, via `RenderSurfaceExtensions.ReplaceWith` → BCnEncoder.Net 2.2.1) or an explicit `format`; `allowResize` (default true) takes the new dims from the image. Reports old/new dims+format, source/record bytes and blocks used. Prep the copy with `bake/scripts/prep_dat.py` (zeroed free arena) until the DRW contiguous-free-list allocator bug is fixed upstream. Writes to `~/ac_base_dats` refused |
| `surface-texture-collapse` | `datPath, idHex, keepDid?, dryRun?` \| `collapses[{idHex,keepDid?}]` | ✓ | `CmdSurfaceTextureCollapse` | Rewrite a SurfaceTexture (0x05) texture list to a single entry (default `keepDid` = the **last** entry = the base-detail level that ships in `client_portal.dat`), so retail `ImgTex::GetSurfaceDID` takes the `m_num==1` branch and cannot fall back to the `client_highres.dat` index-0 original. Reports the old and new lists. Same DAT-copy prep + `~/ac_base_dats` refusal |
