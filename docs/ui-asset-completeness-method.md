...
| 2026-05-24 | (this doc, Wave 7.9.B) | Clothing II Phase D.3 + D.4 — player mesh in viewport + Shift+drag whole-mesh preview. DyeViewport widens to 360×280 + gains `playerRigRoot` at half-scale next to pedestal; `loadPlayerMesh` uses local player's CURRENT substitutions as reference frame. inventory.js dragover detail gains shiftKey/altKey/ctrlKey. Plugin Shift+drag routes through `applyWholeMeshPreview` → `applyAppearance` on local player's main-scene rig with per-session stash for clean revert on hide/drag-end. Diag: `dyePreviewWholeMeshApplied` + `dyePreviewWholeMeshReverted` counters (apply+revert net-zero invariant). |
| 2026-05-24 | (this doc, Wave 7.9.A) | Clothing II Phase D.2 — 3D rotating dye-preview viewport. New `ui/ac_dye_viewport.js::DyeViewport` class — small THREE.js scene (280×280) with cylinder pedestal + rotating rig + auto-framed camera. Borrows existing animationCache + fetchEntitySurfacesPixels (byte-parity with spawn render). Plugin tooltip widened to 280×280 + replaces flat canvas with viewport; falls through to flat canvas when rig can't build. Dispose-on-hide releases WebGL context. New `resolveDyeTriples` helper extracted from `composeDyePreview`. `__diag.clothing.recentDyePreviewsShown[].mode` distinguishes "viewport-3d" vs "flat-fallback". |
| 2026-05-24 | (this doc, Wave 7.9) | Clothing II Phase D — dye-preview plugin (re-scoped from "picker" to "preview-on-drag" per design correction: color is determined by (dye_pot, armor) pair via server recipe, not arbitrary player picking). `inventory.js` adds `dragover` listeners that dispatch `hb:inventory-drag-over` events. New `plugins/dye-preview.js` subscribes + renders a tooltip with `composeDyePreview` output when a known dye-pot wcid is dragged over a dyeable armor; falls back gracefully when armor metadata is missing from wire. `__diag.clothing.{dyePreviewsShown, dyePreviewsShownByReason, recentDyePreviewsShown}` observability. Hardcoded `DYEPOT_OUTCOMES` table covers the 9 canonical dye pots for MVS; full ACE recipe extraction deferred to Phase D.1. |
| 2026-05-24 | (this doc, Wave 7.8) | Clothing II Phase C — CPU preview compositor. New wasm `fetch_dye_preview_pixels(surface_texture_did, sub_palettes)` takes a SurfaceTexture (0x05) directly + composes via byte-identical overlay loop. `ui/ac_dye_preview.js::composeDyePreview(clothingId, setupDid, paletteTemplate, shade)` walks ClothingTable → CloObjectEffect → pickPaletteForShade → wasm call → `<canvas>`. Two-layer (previewCache + inFlight) LRU-256 caching. `__diag.clothing.{dyePreviewsRendered, dyePreviewCacheHits, dyePreviewFailures, recentDyePreviews}` observability. Closes Phase C of `handoff-clothing-table-2026-05-24.md` § C. Fixes two load-bearing bugs surfaced during verification: `fetchEntitySurfacesPixels` missing from `window.__hbWasm`; sp.free() happening before sp.width/sp.height reads in the diag hook (wasm-bindgen "null pointer passed to rust"). |
