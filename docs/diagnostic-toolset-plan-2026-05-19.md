...
| 2 | Event completeness (sounds + particles) | ◐ | `Region.sound_info` + `AmbientSTBDesc` + `AnimationHook` + `PhysicsScript` + `GameMessageSound 0xF750` | `validate_event_completeness.cjs` (shipped; F.D-fu probes still open — drift on AmbientRuntime + PhysicsScript timing) |
...
| Brick | Owner | Files touched | Acceptance |
|---|---|---|---|
| **W3.A — `physics-replay-trace`** (live-ACE) — login to local ACE via Playwright as `phaseN_diag_<runId>` (rotating per run); capture per-tick trace from `window.__predLastPos` / `__predFirstPos` / `__predTickCount`; replay through C# port of `acclient.c::CPhysicsObj::*` predicates; compare per-tick. | **SHIPPED 2026-05-19 (infrastructure) / DRIFT-GAP DOCUMENTED** | `CommandEngine.PhysicsParity.cs` (extended +400 LOC), `apps/holtburger-web/fixtures/physics/probe-scenario.json`, `capture_physics_replay.cjs`, `validate_physics_replay.cjs`, `JsonCommandProcessor.cs` dispatch | Infrastructure stable across 5 consecutive runs (`phaseN_diag_w3a_run{1..5}` accounts, 1025-1028 ticks each). Short-interval drift consistent: maxDrift 2.81-2.89 m, meanDrift 0.38-0.47 m. **Acceptance bar (≤ 0.10 m max + 100% on-ground match) NOT met** — the wasm side's `getLocalPlayerPose()` returns the server-authoritative pose, not a pure prediction stream, so per-tick comparison surfaces server-vs-client integrator gap rather than pure integrator parity. Documented as a Rust-side instrumentation follow-on in [`physics-parity-method.md`](physics-parity-method.md) §"Wave 3.A live-replay" — needs a wasm export of pre-reconciliation prediction (e.g. `__predLastPredictedPos` distinct from `__predLastPos`). 5-run cohort under `/mnt/wbterminal1/holtburger-validator-reports/physics-replay/2026-05-20T0[0-4]-*_w3a_run{1..5}_*/`. |
...
| Brick | Owner | Files touched | Acceptance |
|---|---|---|---|
| **W4.A — `chorizite-decode-surface-chunk <startId> <endId>`** — invoke Chorizite.DatReaderWriter's Surface chain → RenderSurfaceImporter for every Surface in the half-open range; emit PNG + sha256 of pixels per record + a chunk-level `progress.json`. Wasm export `fetch_surface_pixels` is the holtburger-side mirror — driven via headless capture loop. | open | `CommandEngine.TextureParity.cs`, `validate_texture_decode.cjs`, chunk dispatcher in `scripts/wave4_sweep.cjs` | All ~15,318 Surfaces: PNG sha256 match ≥99.5% (~76 tolerance budget for known palette-edge cases); failures clustered into ≤5 categories with documented root causes |
| **W4.B — `chorizite-decode-texture-chain-chunk`** — full Surface→SurfaceTexture→Texture walk including Palette + ClipMap branches across the same record range. Compares mean RGBA against our textured-mean path. | open | same partial | Whole-DAT walk: 100% match on mean RGBA (textured-mean is a fixed integer-math reduction; any drift = parser bug). Failures land in fixture cache for later root-cause sessions. |
| **W4.C — `mesh-vs-obj-export-chunk`** — leverage existing `obj-export` to dump every GfxObj + SetupModel (~15,318 GfxObjs); compare vertex / index / normal arrays against wasm `fetch_model_mesh` output. Chunked + sha-cached like W4.A. | open | `CommandEngine.MeshParity.cs`, `validate_mesh_parity.cjs` | Whole-DAT walk: position/normal/UV arrays byte-identical ≥99% (allowing for known degenerate-triangle filter from `project_emit_dynamic_site` "GfxObj polygon parser stipple/cull bit-mask bug" rev) |
| **W4.D — `env-cell-vs-setup-model-chunk`** — for every EnvCell record in `client_cell_1.dat` (568 in Academy + many more in dungeons): compare retail's CEnvCell layout (per `acclient.c::CEnvCell`) against our `holtburger-dat::env_cell` output. | open | same partial | 100% decode parity; surface IDs match; portal counts match |
| **W4.E — Sweep orchestrator** — `scripts/wave4_sweep.cjs` drives chunks in parallel (4-wide), tracks progress, retries on transient failures, emits aggregate report. Runs as a long-lived background job; status pollable via `wave4-status` WB.Terminal command. | open | `scripts/wave4_sweep.cjs`, `CommandEngine.TextureParity.cs:WaveStatus` | First-pass completes in ≤6 hours on the GTX 1070 box; warm re-run reads from sha-cache in ≤30 seconds |
