...
- **As of 2026-05-20: 14 of 14 surfaces validator-covered** (12 ✓ + W3.A live-replay still post-W3.F refinement; events row 2 ◐→✓ via F.D-fu1..4 closeouts 2026-05-20). Waves 1-5 all SHIPPED. `diag-run-all` is the capstone meta-command.
...
| 2 | Event completeness (sounds + particles) | ✓ | `Region.sound_info` + `AmbientSTBDesc` + `AnimationHook` + `PhysicsScript` + `GameMessageSound 0xF750` | `validate_event_completeness.cjs` + F.D-fu1..4 closeouts SHIPPED 2026-05-20. **All 4 exercised channels firing**: OneOff 3/3, GameMessageSound 2/2, AmbientRuntime probabilistic 9-17/run (matched), PhysicsScriptHook 3/3 (matched by emitter_did). Last 3 runs all PASS (commit pending). F.D-fu1 `window.__synthGameMessageSound` ships in `scene3d/index.js` (alongside `__playWave`); F.D-fu2 `AmbientRuntime.setClockForTest` + wall-clock dt derivation; F.D-fu3 `EntityManager.awaitSpawnResolution` + `awaitParticleChainResolution` descriptor returns; F.D-fu4 60s default probe window. Validator additionally: PROBE_MTABLE_DID arg-plumbing fix into spawn evaluate closure, contract-level PhysicsScriptHook record push pre-await of slow addEmitter, fire-and-forget addEmitter visual rendering, emitter_did keyed matching (replaces the broken wall-clock anchor logic for start_time_s=0 hooks). Method doc: [`event-completeness-method.md`](event-completeness-method.md). AnimationHook + SkyChain remain documented deferred per F.B.5 + scope; not blocking ◐→✓. |
...
| 5 | **DAT parser parity (~20 file types)** | ✓ | `Chorizite.DatReaderWriter` NuGet v2.1.2 (53 generated DBObj types) + `~/ac_base_dats/` sha-pinned | `validate_dat_parity.cjs --phase=both` + `chorizite-{list-dat-records,parse-dat-record,list-dat-types}` + `cargo run --example parse_dat_record`. **Wave 2.A+2.B+2.D SHIPPED 2026-05-19; EnvCell FAIL closed 2026-05-20**: Phase A 24/24 × 906/906 structural PASS; Phase B **11 PASS / 13 GAP / 0 FAIL** (was 11/12/1; EnvCell `visibleCells[]` "ordering drift" root cause identified as `CellPortal` struct missing `polygon_id: u16` field — 8 bytes per portal not 6 — which silently shifted all subsequent reads. Also fixed: env_cell.rs flag-mask shift bug (0x01/0x02 → 0x02/0x08) and parse_dat_record example dispatch ambiguity for cell-DAT IDs with prefix bytes that collide with portal-DAT type-prefixes like 0x20/SoundTable. See [[project_envcell_flagmask_fix_2026-05-20]] for the full audit). Remaining EnvCell GAP is Chorizite-side property-graph nulls (`cellId`, `portals`) + field-naming drift (`portals` vs `cellPortals`, `position` vs `frame`). See [`dat-parity-method.md`](dat-parity-method.md). |
...
| 11 | **Mesh / triangulation parity** (GfxObj + SetupModel + EnvCell) | ✓ | WB.Terminal `obj-export` ground truth + ACE `EnvCell.cs` + retail `CGfxObj::*` | `validate_mesh_parity.cjs` + `mesh-vs-obj-export-chunk` + `env-cell-vs-setup-model-chunk` — **Wave 4.C+D SHIPPED 2026-05-20**. **Cold full-DAT sweep 2026-05-20: mesh 17,740 records PASS (17,655 GfxObj + 85 Setup) across 31 chunks in 18.2 s; env-cell 735,544 records PASS across 1,470 chunks in 55.6 s.** Status histogram `{ok: total, parseError: 0, missing: 0}` across all chunks. **1 knownDrift** = pre-existing W2.D `EnvCell.visibleCells[]` ordering on `0x72040335` (counted, not FAIL — sweep ran against pre-envcell-flagmask-fix state, md5 `9a15a89810941e1a802f0443bca02e28`). Method doc: [`mesh-parity-method.md`](mesh-parity-method.md). |
...
| **W2.D — Rust serde-Serialize derives + `parse_dat_record` example binary + Phase-B field-tree diff** | 2026-05-19 ✓ (+ EnvCell follow-on closed 2026-05-20) | `crates/holtburger-dat/src/{file_type/*,graphics,physics,landblock}.rs` (~50 `#[derive(Serialize)]` adds across 26 files), `crates/holtburger-dat/examples/parse_dat_record.rs` (~290 LOC), `validate_dat_parity.cjs` extended (~290 → ~620 LOC) | **Shipped 2026-05-19**: Phase B = 11 PASS / 12 GAP / 1 FAIL. **2026-05-20 EnvCell FAIL closed → 11 PASS / 13 GAP / 0 FAIL** (records: pass 110 / drift 0 / gap 76 / infra-fail 0). The "visibleCells ordering drift" root cause was a missing field in the `CellPortal` struct (`polygon_id: u16` not declared, so the parser read 6 bytes/portal instead of 8), compounded by env_cell.rs's flag-mask shift (0x01/0x02 in code vs 0x02/0x08 per ACE+DRW+acclient.c) and a parse_dat_record dispatcher ambiguity that routed cell-DAT id `0x200F0A0B` through `SoundTable::unpack`. All three fixed; see [[project_envcell_flagmask_fix_2026-05-20]]. Remaining EnvCell GAP is Chorizite-side DBObj property-graph nulls + field-naming drift, both documented in [[project_envcell_visiblecells_investigation_2026-05-20]]. Other 2026-05-19 findings still apply: System.Text.Json u32→i32 quirk (handled); DRW property graph elides wire bits in Surface/SurfaceTexture/Environment/RenderSurface (tagged `chorizite-zeroed`); count-fields re-derived by DRW (exempt). |
