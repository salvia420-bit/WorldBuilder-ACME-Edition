...
| **Wave 7.9.B** | **Clothing II Phase D.3 + D.4 — player mesh in DyeViewport at half-scale next to pedestal (uses local player's CURRENT substitutions as reference); Shift+drag-over routes through `applyAppearance` on local player's main-scene rig with per-session stash for clean revert on hide. Diag: `dyePreviewWholeMeshApplied` + `dyePreviewWholeMeshReverted` net-zero invariant. Closes Clothing II handoff in full.** | **2026-05-24** | Updated [`handoff-clothing-table-2026-05-24.md`](handoff-clothing-table-2026-05-24.md) § C |
| **Wave 7.9.A** | **Clothing II Phase D.2 — 3D rotating dye-preview viewport. New `ui/ac_dye_viewport.js::DyeViewport` (THREE.js mini-scene 280×280 with cylinder pedestal + rotating rig) replaces the flat canvas in the dye-preview tooltip. Borrows animationCache + fetchEntitySurfacesPixels for byte-parity with spawn render. Dispose-on-hide releases WebGL context (Chrome caps ~16). `__diag.clothing.recentDyePreviewsShown[].mode` distinguishes "viewport-3d" vs "flat-fallback".** | **2026-05-24** | Updated [`handoff-clothing-table-2026-05-24.md`](handoff-clothing-table-2026-05-24.md) § C |
| **Wave 7.9** | **Clothing II Phase D — dye-preview tooltip plugin. `inventory.js` gains `dragover` event dispatch; new `plugins/dye-preview.js` subscribes + renders composeDyePreview output as a tooltip near the cursor when a dye pot is dragged over dyeable armor. Falls back to "missing metadata" message when armor wire packet omits clothingBaseId/setupId. `__diag.clothing.{dyePreviewsShown, dyePreviewsShownByReason, recentDyePreviewsShown}` observability. Hardcoded `DYEPOT_OUTCOMES` for 9 canonical dye pots; full ACE recipe extraction is the D.1 follow-on.** | **2026-05-24** | Updated [`handoff-clothing-table-2026-05-24.md`](handoff-clothing-table-2026-05-24.md) § C |
| **Wave 7.8** | **Clothing II Phase C — CPU dye-preview compositor: new `fetch_dye_preview_pixels` wasm + `ui/ac_dye_preview.js::composeDyePreview` + LRU-256 two-layer caching + `__diag.clothing.{dyePreviewsRendered, dyePreviewCacheHits, dyePreviewFailures, recentDyePreviews}` observability. Closes Phase C of handoff § C.** | **2026-05-24** | Updated [`handoff-clothing-table-2026-05-24.md`](handoff-clothing-table-2026-05-24.md) § C |
| **Wave 7.7** | **Clothing II foundation (Phases A + B): `__diag.clothing.{dyeApplications,recentDyes,dyesBySource}` hooks at fetchEntitySurfacesPixels callsites; `fetch_palette` wasm export + `ui/ac_palette.js` runtime + `__diag.palettes.{palettesLoaded,paletteFailures}`. Wiki-grounded: server-pushed dye is W7.3 already; W7.7 makes it auditable + adds the reader plumbing for future preview compositor (Phase C) + picker UI (Phase D).** | **2026-05-24** | Updated [`handoff-clothing-table-2026-05-24.md`](handoff-clothing-table-2026-05-24.md) § C |
