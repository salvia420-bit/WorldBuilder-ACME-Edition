...
1. Resume training from `pipeline_data/models/resume.pt`
2. Let it run for a bounded slice such as `25-50` epochs
3. Run the fixed small-region probe against the latest inference-loadable checkpoint
4. Inspect `raw_generated`, `accepted_after_validation`, `PAD`, and `STOP`
5. If `raw_generated == 0`, stop and debug locally instead of running full-world generation
